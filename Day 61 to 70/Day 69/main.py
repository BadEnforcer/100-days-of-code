...
class User(db.Model, UserMixin):
    ...
    def get(self, u_email):
        user = db.session.query(User).filter_by(u_email).first()
        return user.id
...
def register():
    ...
    if form.validate_on_submit():
        if db.session.query(User).filter_by(email=form.email.data).first():
            flash('Uh oh!, account already exists, please login.')
            return redirect(url_for('get_all_posts', logged_in=current_user.is_authenticated))
        new_usr = User()

        new_usr.name = form.name.data
        new_usr.email = form.email.data
        new_usr.password = generate_password_hash(form.password.data)
        with app.app_context():
