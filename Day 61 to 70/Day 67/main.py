...
def edit_post(post_id):
    ...
    if edit_form.validate_on_submit():
        with app.app_context():
            ...
            # old title: losing control
            # new title: losing control 2

            post_to_update.title = edit_form.title.data

            print(edit_form.title.data)  # prints 'losing control 2'
            print(request.form.get('title'))  # prints 'losing control 2'
            ...
            # commit changes
            db.session.commit()  # after the commit command data is not updated stll
            return redirect(url_for("get_all_posts"))


@app.route('/remove-post/<int:post_id>', methods=['GET', 'POST'])
def delete(post_id):
